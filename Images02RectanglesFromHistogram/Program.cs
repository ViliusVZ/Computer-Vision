...
namespace Images
{
    public static class Histograms
    {
        private static void Main(string[] args)
        {
            ...
            do
            {
                if (capture.Read(capturedImage))
                {
                    ...
                    using (var bgrHistogram = MakeBgrHistogram(new Mat(new Size(256, 110), MatType.CV_8UC3, Scalar.White), boxImage, out var histogramMeasure))
                    {
                        if (Cv2.WaitKey(1) == 's')
                        {
        ...
        private static Mat MakeBgrHistogram(Mat histogram, Mat image, out float[] histogramMeasure)
        {
